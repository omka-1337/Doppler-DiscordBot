...
Everything else — module toggles, command prefix, AI persona/prompt, translator provider and keys, moderation and mod-log settings, temp-voice-channel IDs, music worker bot tokens, Server Protect's age/role/raid settings — lives in the SQLite database (`savedata/bot.db`) and is edited entirely through the web dashboard. Uploaded embed images are stored in `savedata/embeds/images/`.
